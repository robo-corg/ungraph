...
impl Model for Safetensors {
    fn summary<'a>(&'a self, filename: Option<&'a str>) -> Box<dyn Summary + 'a> {
        let architecture = self.metadata.get("modelspec.architecture").and_then(|v| v.as_str());
        let implementation = self.metadata.get("modelspec.implementation").and_then(|v| v.as_str());
